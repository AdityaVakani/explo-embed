...
export function ClinicFilter({ values, options, onChange, disabled }: ClinicFilterProps) {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative flex flex-col gap-1 text-xs font-medium uppercase tracking-wide text-slate-700 ${open ? 'z-40' : 'z-20'}`}
    >
      ...
      {open ? (
        <div className="absolute right-0 top-full z-50 mt-2 w-72 rounded-lg border border-slate-200 bg-white p-3 text-xs shadow-xl">
          <input
            ref={searchInputRef}
            type="text"
            value={searchTerm}
